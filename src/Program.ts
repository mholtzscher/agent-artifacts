...
const HttpLive = Layer.unwrap(Effect.map(AppConfigService, (config) => BunHttpServer.layer({ port: config.port })));
...
BunRuntime.runMain(
  Effect.log("agent-artifacts booting").pipe(Effect.andThen(Layer.launch(MainLive)), Effect.provide(TelemetryLive)),
);
